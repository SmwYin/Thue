...
by
  simp only [zero_def, mul_def, zero_mul, mul_zero, _root_.add_zero]
  ring

lemma mul_zero : a * 0 = 0 :=
by
  simp only [zero_def, mul_def, mul_zero, _root_.add_zero]
  ring

lemma one_mul : 1 * a = a :=
by
  simp only [one_def, mul_def, one_mul, MulZeroClass.mul_zero, MulZeroClass.zero_mul, _root_.add_zero]
  ring

lemma mul_one : a * 1 = a :=
by
  simp only [one_def, mul_def, mul_one, MulZeroClass.mul_zero, _root_.add_zero, _root_.zero_add]
  ring
...
by
  simp only [neg_def, add_def, add_left_neg, zero_def]
  ring
...
lemma α_cube: α ^ 3 = (2:R) :=
by
  simp only [α, pow_three, mul_def, MulZeroClass.mul_zero, _root_.mul_one, _root_.add_zero, _root_.zero_add]
  exact rfl


lemma α_cube': α ^ 3 = (2:ℕ) :=
by
  rw [α_cube]
  exact rfl

lemma α_pow_four: α ^ 4 = (2:R) * α :=
by
  have h: α ^ 4 = α ^ 3 * α
  · ring
  rw [h]
...
lemma α_pow_four': α ^ 4 = (2:ℕ) * α :=
by
  rw [α_pow_four]
  congr

lemma α_pow_five : α ^ 5 = (2:R) * α^2 :=
by
  have h: α ^ 5 = α ^ 3 * α ^ 2
  · ring
  rw [h]
...
lemma α_pow_five' : α ^ 5 = (2:ℕ) * α^2 :=
by
  rw [α_pow_five]
  congr

lemma α_pow_six : α ^ 6 = (4:R) :=
by
  have h: α ^ 6 = α ^ 3 * α ^ 3
  · ring
  rw [h]
...
by
  ...
  | ofNat x =>
    induction x with
    | zero =>
      simp only [Nat.zero_eq, Int.ofNat_eq_coe, CharP.cast_eq_zero, Int.cast_zero]
      rfl
    | succ n ih =>
      simp only [Int.ofNat_eq_coe, Int.cast_ofNat] at ih
      simp only [Int.ofNat_eq_coe, Nat.cast_succ, Int.cast_add, Int.cast_ofNat, Int.cast_one, one_def, add_def,
        _root_.add_zero, add_left_inj, ih]
  | negSucc x =>
    induction x with
    | zero =>
    ...
    | succ n ih =>
      ...
      simp only [Int.cast_negSucc, Nat.cast_add, Nat.cast_succ, Nat.cast_one, neg_add_rev]
      rw [ih]
...
lemma xcomp (x:ℤ) (y:ℤ) (z:ℤ) : (x + y * α + z * α^2).x = x :=
by
  simp only [add_def, sq, mul_def, αx, αy, αz, Int_Cast_R]
  ring

lemma ycomp (x:ℤ) (y:ℤ) (z:ℤ) : (x + y * α + z * α^2).y = y :=
by
  simp only [add_def, sq, mul_def, αx, αy, αz, Int_Cast_R]
  ring
...
lemma xcomp' (x:ℤ) (y:ℤ) (z:ℤ) : (x + α * y + α^2 * z).x = x :=
by
  simp only [add_def, sq, mul_def, αx, αy, αz, Int_Cast_R]
  ring

lemma ycomp' (x:ℤ) (y:ℤ) (z:ℤ) : (x + α * y + α^2 * z).y = y :=
by
  simp only [add_def, sq, mul_def, αx, αy, αz, Int_Cast_R]
  ring
...
lemma gtone2: (1:ℝ) < ((2:ℝ) ^ ((1:ℝ) / (3:ℝ))) ^ 2 :=
by
  rw [one_lt_sq_iff₀ hnonneg]
  exact gtone
...
by
  ...
  _ = -ω^2 - ω    := by ring
  _ = -(-ω-1) - ω := by simp only [omega_sq, neg_sub, sub_neg_eq_add, add_sub_cancel]
  _ = _           := by ring
...
by
  ...
  _ = ω ^ (3 * 2) := by ring
  _ = (ω ^ 3) ^ 2 := by ring
  _ = _ := by simp only [omega_pow_three, one_pow, Nat.cast_one]

lemma absomega₂: Complex.abs (ω) = (1:ℝ):=
by
  simp only [Complex.abs_apply, Complex.normSq_apply, ω_re, ω_im, Real.sqrt_eq_one]
  norm_num
  rw [div_mul_div_comm]
  rw[← sq]
  simp only [rt3_sq]
...
lemma absomega₁: Complex.abs (-ω-1) = (1:ℝ) :=
by
  simp only [Complex.abs_apply, Complex.normSq_apply, Complex.sub_re, Complex.neg_re, ω_re, Complex.one_re, Complex.sub_im, Complex.neg_im, ω_im, Complex.one_im, sub_zero, mul_neg, neg_mul, neg_neg, Real.sqrt_eq_one]
  norm_num
  rw [div_mul_div_comm]
  rw[← sq]
  simp only [rt3_sq]
...
by
  simp only [α₂]
  calc
  _ = α₁^ 3 * ω ^ 3 := by ring; push_cast; ring
...
by
  ...
  calc
  _ = α₁ ^ 3 * (ω ^ 2) ^ 3 := by ring; norm_cast; ring
  _ = α₁ ^ 3 * ω ^ 6       := by ring
...
lemma xcompeq {x:R} {y:R} (h: x = y) : x.x = y.x :=
by
  rw [h]

lemma ycompeq {x:R} {y:R} (h: x = y) : x.y = y.y :=
by
  rw [h]
...
def σ₁ : R →+* ℝ where
  ...
  map_mul'  := by intro x y; simp only [mul_def, Int.cast_add, Int.cast_mul]; ring; simp only [alpha₁_pow_three, alpha₁_pow_four]; ring
...
@[simp]
lemma fundx : fund.val.x = 1 :=
by
  rfl

@[simp]
lemma fundy : fund.val.y = 1 :=
by
  rfl

@[simp]
lemma fundz : fund.val.z = 1 :=
by
  rfl
...
by
  ...
  rw [h, h2]
  ext <;>
  · simp only [Nat.cast_one,mul_def, fundx, fundy, fundz]
    ring
...
by
  ...
  · intro r
    ...
    | succ r ih =>
      ...
      -- Want to show: the α² coefficient (which is equal to zero) is congruent to -2×3ʳa mod 3ʳ⁺¹
      · sorry
      obtain ⟨v, hv⟩ := h0
      rw [h1] at hv
      have hz:= zcomp x (-y) 0
