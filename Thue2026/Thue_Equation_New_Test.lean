...
noncomputable def cubic : ℤ[X] := X ^ 3 - C 2

lemma cubic_monic : cubic.Monic := by
  simpa [cubic] using
    (monic_X_pow_sub_C (R := ℤ) (a := (2 : ℤ)) (show (3 : ℕ) ≠ 0 by decide))

lemma cubic_natDegree : cubic.natDegree = 3 := by
  simpa [cubic] using (natDegree_X_pow_sub_C (R := ℤ) (n := 3) (r := (2 : ℤ)))

abbrev A := AdjoinRoot cubic

noncomputable def alphaAdj : A := AdjoinRoot.root cubic

noncomputable abbrev pb : PowerBasis ℤ A :=
  AdjoinRoot.powerBasis' (R := ℤ) (g := cubic) cubic_monic

lemma pb_dim : pb.dim = 3 := by
  simpa [pb, cubic_natDegree]

noncomputable def basis3 : Module.Basis (Fin 3) ℤ A :=
  pb.basis.reindex (finCongr pb_dim)

lemma basis3_zero : basis3 0 = (1 : A) := by
  have h : basis3 0 = pb.basis ((finCongr pb_dim).symm 0) := by
    convert (pb.basis.reindex_apply (finCongr pb_dim) 0) using 1
  have hi : (((finCongr pb_dim).symm 0 : Fin pb.dim) : ℕ) = 0 := by
    simpa using (finCongr_symm_apply_coe pb_dim 0)
  rw [h, pb.basis_eq_pow, hi]
  simp [alphaAdj]

lemma basis3_one : basis3 1 = alphaAdj := by
  have h : basis3 1 = pb.basis ((finCongr pb_dim).symm 1) := by
    convert (pb.basis.reindex_apply (finCongr pb_dim) 1) using 1
  have hi : (((finCongr pb_dim).symm 1 : Fin pb.dim) : ℕ) = 1 := by
    simpa using (finCongr_symm_apply_coe pb_dim 1)
  rw [h, pb.basis_eq_pow, hi]
  simp [alphaAdj]

lemma basis3_two : basis3 2 = alphaAdj ^ 2 := by
  have h : basis3 2 = pb.basis ((finCongr pb_dim).symm 2) := by
    convert (pb.basis.reindex_apply (finCongr pb_dim) 2) using 1
  have hi : (((finCongr pb_dim).symm 2 : Fin pb.dim) : ℕ) = 2 := by
    simpa using (finCongr_symm_apply_coe pb_dim 2)
  rw [h, pb.basis_eq_pow, hi]
  simp [alphaAdj]

noncomputable def coords (a : R) : Fin 3 →₀ ℤ :=
  Finsupp.single 0 a.x + Finsupp.single 1 a.y + Finsupp.single 2 a.z

noncomputable def toAdjoinRoot (a : R) : A := basis3.repr.symm (coords a)

noncomputable def ofAdjoinRoot (a : A) : R := ⟨basis3.repr a 0, basis3.repr a 1, basis3.repr a 2⟩

lemma left_inv : Function.LeftInverse ofAdjoinRoot toAdjoinRoot := by
  intro a
  ext <;> rw [ofAdjoinRoot, toAdjoinRoot, LinearEquiv.apply_symm_apply] <;> simp [coords]

lemma right_inv : Function.RightInverse ofAdjoinRoot toAdjoinRoot := by
  intro a
  apply basis3.repr.injective
  rw [toAdjoinRoot, LinearEquiv.apply_symm_apply]
  ext i
  fin_cases i <;> simp [coords, ofAdjoinRoot]

noncomputable def equiv : R ≃ A where
  toFun := toAdjoinRoot
  invFun := ofAdjoinRoot
  left_inv := left_inv
  right_inv := right_inv

lemma toAdjoinRoot_eq (a : R) :
    toAdjoinRoot a = a.x + a.y * alphaAdj + a.z * alphaAdj ^ 2 := by
  rw [toAdjoinRoot]
  simp [coords, basis3_zero, basis3_one, basis3_two, add_assoc]

lemma alphaAdj_cube : alphaAdj ^ 3 = (2 : A) := by
  have h : aeval alphaAdj cubic = 0 := by
    simpa [alphaAdj] using (AdjoinRoot.aeval_eq (f := cubic) cubic)
  have h' : alphaAdj ^ 3 - (2 : A) = 0 := by
    simpa [cubic, aeval_def] using h
  exact sub_eq_zero.mp h'

lemma alphaAdj_four : alphaAdj ^ 4 = (2 : A) * alphaAdj := by
  calc
    alphaAdj ^ 4 = alphaAdj ^ 3 * alphaAdj := by
      simp [pow_succ, mul_comm, mul_left_comm, mul_assoc]
    _ = (2 : A) * alphaAdj := by rw [alphaAdj_cube]
...
instance : Pow R ℕ := ⟨fun a n => npowRec n a⟩
instance : SMul ℕ R := ⟨fun n a => nsmulRec n a⟩
instance : SMul ℤ R := ⟨fun n a => zsmulRec nsmulRec n a⟩
instance : NatCast R := ⟨fun n => ⟨n, 0, 0⟩⟩
instance : IntCast R := ⟨fun z => ⟨z, 0, 0⟩⟩
...
lemma toAdjoinRoot_zero : toAdjoinRoot (0 : R) = 0 := by
  simp [zero_def, toAdjoinRoot_eq]

lemma toAdjoinRoot_one : toAdjoinRoot (1 : R) = 1 := by
  simp [one_def, toAdjoinRoot_eq]

lemma toAdjoinRoot_add (a b : R) : toAdjoinRoot (a + b) = toAdjoinRoot a + toAdjoinRoot b := by
  simp [add_def, toAdjoinRoot_eq, add_assoc, add_left_comm, add_comm]
  ring_nf

lemma toAdjoinRoot_neg (a : R) : toAdjoinRoot (-a) = -toAdjoinRoot a := by
  simp [neg_def, toAdjoinRoot_eq]
  ring_nf

lemma toAdjoinRoot_mul (a b : R) : toAdjoinRoot (a * b) = toAdjoinRoot a * toAdjoinRoot b := by
  simp [mul_def, toAdjoinRoot_eq, add_assoc, add_left_comm, add_comm]
  ring_nf
  rw [alphaAdj_cube, alphaAdj_four]
  ring_nf

lemma toAdjoinRoot_sub (a b : R) : toAdjoinRoot (a - b) = toAdjoinRoot a - toAdjoinRoot b := by
  rw [show a - b = a + -b by rfl, toAdjoinRoot_add, toAdjoinRoot_neg]
  simpa [sub_eq_add_neg]

lemma toAdjoinRoot_npowRec (a : R) : ∀ n : ℕ, toAdjoinRoot (npowRec n a) = toAdjoinRoot a ^ n
  | 0 => by
      simp [npowRec, toAdjoinRoot_one]
  | n + 1 => by
      rw [npowRec, toAdjoinRoot_mul, toAdjoinRoot_npowRec a n]
      simpa [pow_succ]

lemma toAdjoinRoot_npow (a : R) (n : ℕ) : toAdjoinRoot (a ^ n) = toAdjoinRoot a ^ n := by
  simpa using toAdjoinRoot_npowRec a n

lemma toAdjoinRoot_nsmulRec (a : R) : ∀ n : ℕ, toAdjoinRoot (nsmulRec n a) = n • toAdjoinRoot a
  | 0 => by
      simp [nsmulRec, toAdjoinRoot_zero]
  | n + 1 => by
      rw [nsmulRec, toAdjoinRoot_add, toAdjoinRoot_nsmulRec a n]
      simpa [nsmul_eq_mul] using (succ_nsmul (a := toAdjoinRoot a) (n := n)).symm

lemma toAdjoinRoot_nsmul (n : ℕ) (a : R) : toAdjoinRoot (n • a) = n • toAdjoinRoot a := by
  simpa using toAdjoinRoot_nsmulRec a n

lemma toAdjoinRoot_zsmul (n : ℤ) (a : R) : toAdjoinRoot (n • a) = n • toAdjoinRoot a := by
  cases n with
  | ofNat n =>
      change toAdjoinRoot (zsmulRec nsmulRec (Int.ofNat n) a) = (Int.ofNat n) • toAdjoinRoot a
      simpa [zsmulRec] using toAdjoinRoot_nsmulRec a n
  | negSucc n =>
      change toAdjoinRoot (zsmulRec nsmulRec (Int.negSucc n) a) = (Int.negSucc n) • toAdjoinRoot a
      rw [zsmulRec, toAdjoinRoot_neg, toAdjoinRoot_nsmulRec]
      simpa using (negSucc_zsmul (toAdjoinRoot a) n).symm

lemma toAdjoinRoot_natCast (n : ℕ) : toAdjoinRoot (n : R) = (n : A) := by
  change toAdjoinRoot ⟨n, 0, 0⟩ = (n : A)
  simp [toAdjoinRoot_eq]

lemma toAdjoinRoot_intCast (n : ℤ) : toAdjoinRoot (n : R) = (n : A) := by
  change toAdjoinRoot ⟨n, 0, 0⟩ = (n : A)
  simp [toAdjoinRoot_eq]

noncomputable def ringEquiv : R ≃+* A where
  toFun := toAdjoinRoot
  invFun := ofAdjoinRoot
  left_inv := left_inv
  right_inv := right_inv
...
noncomputable instance : CommRing R :=
  Function.Injective.commRing toAdjoinRoot equiv.injective
    toAdjoinRoot_zero toAdjoinRoot_one toAdjoinRoot_add toAdjoinRoot_mul
    toAdjoinRoot_neg toAdjoinRoot_sub toAdjoinRoot_nsmul toAdjoinRoot_zsmul
    toAdjoinRoot_npow toAdjoinRoot_natCast toAdjoinRoot_intCast
...
@[simp]
lemma one_x : (1 : R).x = 1 := by rw [one_def]

@[simp]
lemma one_y : (1 : R).y = 0 := by rw [one_def]

@[simp]
lemma one_z : (1 : R).z = 0 := by rw [one_def]

@[simp]
lemma natCast_x (n : ℕ) : (n : R).x = n := rfl

@[simp]
lemma natCast_y (n : ℕ) : (n : R).y = 0 := rfl

@[simp]
lemma natCast_z (n : ℕ) : (n : R).z = 0 := rfl

@[simp]
lemma intCast_x (x : ℤ) : (x : R).x = x := by rw [Int_Cast_R]

@[simp]
lemma intCast_y (x : ℤ) : (x : R).y = 0 := by rw [Int_Cast_R]

@[simp]
lemma intCast_z (x : ℤ) : (x : R).z = 0 := by rw [Int_Cast_R]

@[simp]
lemma alpha_sq : α ^ 2 = ⟨0, 0, 1⟩ := by
  simp [sq, α, mul_def]

@[simp]
lemma alpha_sq_x : (α ^ 2).x = 0 := by rw [alpha_sq]

@[simp]
lemma alpha_sq_y : (α ^ 2).y = 0 := by rw [alpha_sq]

@[simp]
lemma alpha_sq_z : (α ^ 2).z = 1 := by rw [alpha_sq]
...
by
  simp only [pow_two]
  apply Complex.ext
  · simp only [Complex.mul_re, ω_re, neg_div, one_div, mul_neg, neg_mul, neg_neg, ω_im, Complex.sub_re, Complex.neg_re, Complex.one_re]
    calc
      _ = (4:ℝ) ⁻¹ - √3^2 / 4    := by ring
      _ = 4⁻¹ - 3 / 4            := by simp only [rt3_sq]
      _ = _                      := by ring
  · simp only [Complex.mul_im, ω_re, neg_div, one_div, ω_im, neg_mul, mul_neg, Complex.sub_im, Complex.neg_im, Complex.one_im, sub_zero]
    ring
...
def σ₁ : R →+* ℝ where
  ...
  map_one'  := by simp only [one_def, Int.cast_one, Int.cast_zero, MulZeroClass.zero_mul, _root_.add_zero]
  map_mul'  := by intro x y; simp only [mul_def, Int.cast_add, Int.cast_mul]; ring_nf; simp only [alpha₁_pow_three, alpha₁_pow_four]; ring_nf
  map_zero' := by simp only [zero_def, Int.cast_zero, MulZeroClass.zero_mul, _root_.add_zero]
...
by
  cases n with
  | ofNat _ =>
    simp only [σ₁, α₁, one_div, RingHom.coe_mk, MonoidHom.coe_mk, OneHom.coe_mk, Int.ofNat_eq_natCast, zpow_natCast,
...
by
  simp only [norm, Nat.cast_ofNat, MonoidHom.coe_mk, OneHom.coe_mk, Int.cast_sub, Int.cast_add, Int.cast_pow,
    Int.cast_mul, σ₁, RingHom.coe_mk, Complex.ofReal_add,
...
def fund : Rˣ where
  ...
  inv := α - 1
  val_inv := by
    ext <;> simp [sub_eq_add_neg, sq, α, one_def, add_def, neg_def, mul_def]
  inv_val := by
    ext <;> simp [sub_eq_add_neg, sq, α, one_def, add_def, neg_def, mul_def]

lemma fundx : fund.val.x = 1 := by
  simp [fund, sq, α, one_def, add_def, mul_def]

lemma fundy : fund.val.y = 1 := by
  simp [fund, sq, α, one_def, add_def, mul_def]

lemma fundz : fund.val.z = 1 := by
  simp [fund, sq, α, one_def, add_def, mul_def]
...
by
  change norm (1 + α + α ^ 2) = 1
  simp [norm, sq, α, one_def, add_def, neg_def, mul_def]

lemma norm_fund_inv : norm (fund⁻¹:Rˣ) = 1 :=
by
  change norm (α - 1) = 1
  simp [norm, sub_eq_add_neg, α, one_def, add_def, neg_def, mul_def]
...
by
  simp only [fund, α, σ₁, Nat.cast_one, map_add, map_one, RingHom.coe_mk, MonoidHom.coe_mk, OneHom.coe_mk, Int.cast_zero, Int.cast_one, _root_.one_mul, _root_.zero_add, MulZeroClass.zero_mul, _root_.add_zero, map_pow]
  apply add_pos
  · apply add_pos
...
by
  ...
  _ = _ := by congr; simp only [σ₁, α₁, one_div, RingHom.coe_mk, MonoidHom.coe_mk, OneHom.coe_mk, Complex.ofReal_add, Complex.ofReal_intCast, Complex.ofReal_mul, Complex.ofReal_pow, σ₂, α₂, σ₃, α₃]; ring_nf; simp only [omega_sq]; ring_nf
...
by
  ...
  _ = _ := by congr; simp only [α₁, one_div, σ₁, RingHom.coe_mk, MonoidHom.coe_mk, OneHom.coe_mk, Complex.ofReal_add, Complex.ofReal_intCast, Complex.ofReal_mul, Complex.ofReal_pow, σ₂, α₂, σ₃, α₃]; ring_nf; simp only [omega_sq]; ring_nf
...
by
  ...
  _ = _ := by congr; simp only [Nat.cast_ofNat, α₁, one_div, Complex.ofReal_pow, σ₁, RingHom.coe_mk, MonoidHom.coe_mk, OneHom.coe_mk, Complex.ofReal_add, Complex.ofReal_intCast, Complex.ofReal_mul, σ₂, α₂, σ₃, α₃]; ring_nf; simp only [omega_sq]; ring_nf
...
by
  ...
  | inl h1 =>
    ...
    | inr h2 =>
      ...
      | inl h3 =>
        have nA: norm A = 2
        · simp only [norm, Nat.cast_ofNat, MonoidHom.coe_mk, OneHom.coe_mk, h1, h2, MulZeroClass.mul_zero, h3]; norm_num
        rw[nA] at h
        have neg: ¬ (2:ℤ) = 1
        · push_neg
  ...
  | inr h1 =>
    ...
    | inr h2 =>
      ...
      | inr h3 =>
        right
        ext
        · simpa [h1] using fundx.symm
        · simpa [h2] using fundy.symm
        · simpa [h3] using fundz.symm
...
by
  ...
  | inl h1 =>
    ...
    _ = A * fund ^ n := by
      change (u * fund ^ (-n)) * fund ^ n = A * fund ^ n
      rfl
    _ = 1 * fund ^ n := by rw[h1]
...
by
  have threex: (3:R).x = 3 := by simpa using (intCast_x 3)
  have threey: (3:R).y = 0 := by simpa using (intCast_y 3)
  have threez: (3:R).z = 0 := by simpa using (intCast_z 3)
  have sixx: (6:R).x = 6 := by simpa using (intCast_x 6)
  have sixy: (6:R).y = 0 := by simpa using (intCast_y 6)
  have sixz: (6:R).z = 0 := by simpa using (intCast_z 6)
  ...
  · ext <;>
    · simp only [Int.cast_one, one_def, Int_Cast_R, mul_def, MulZeroClass.mul_zero, _root_.add_zero, MulZeroClass.zero_mul, add_def, α, _root_.mul_one, _root_.one_mul, _root_.zero_add, sq]
  rw [h, h2]
  ext <;>
  · simp only [mul_def]
...
lemma pre_2_final_case_one (b r:ℕ) (hr: 1 ≤ r) : ∃ (x:R), ((fund ^ 3 ^ r) ^ (-1:ℤ)) ^ b = (3:R) ^ (r + 1) * x + ((1:R) + (3:R) ^ r * -↑b * (α ^ 2 - α)) :=
by
  push_cast
  induction b with
  ...
  | succ n ih =>
    obtain ⟨k, hk⟩ := ih
    obtain ⟨v, hv⟩ := pre_pre_2_final_case_one r hr
    set e : R := α ^ 2 - α with he
    use (3^(r-1)*n*e*e+k*3^(r+1)*v+v+3^r*(-n)*e*v+k*(1-3^r*e))
    have hp : (3 : R) ^ (r + 1) * ((3 ^ (r - 1) : ℕ) : R) = (3 : R) ^ r * (3 : R) ^ r := by
      rw [Nat.cast_pow]
      calc
        (3 : R) ^ (r + 1) * (3 : R) ^ (r - 1) = (3 : R) ^ ((r + 1) + (r - 1)) := by
          simp only [← pow_add]
        _ = (3 : R) ^ (r + r) := by
          congr 1
          omega
    ...
    rw [pow_one, hk, hv]
    clear hk hv
    simp only [sub_eq_add_neg, add_mul, mul_add, one_mul, mul_one, neg_add, add_assoc, mul_assoc]
    conv_rhs =>
      rw [← mul_assoc]
      rw [hp]
    simp [Nat.cast_add, Nat.cast_one, Nat.cast_mul, Nat.cast_pow, pow_add, pow_mul, mul_assoc]
    ring
    ...
    | succ r ih =>
      ...
      · set a := (n + 1) / ((3:ℤ) ^ r) with ha
        ...
        · have h0' := mod_prop4 (fund^n).val (α + 3^r*a*(2-α^2) - (1+ 3 ^ r * a * (α^2 - α))) (3 ^ (r+1)) preh0
          ...
          · norm_cast
            rw [zcomp_of_scalar_mul, zcomp_of_sum]
            have hnegSq : (-α ^ 2).z = -1 := by
              norm_num [α, sq, mul_def, neg_def]
            rw [show (2 : R).z = 0 by rfl, hnegSq]
            ring
          have h3: (-((1:ℕ) + ↑(3 ^ r) * ↑a * (α ^ 2 + -α))).z = - (3^r)*a
          · norm_cast
            rw [neg_def, zcomp_of_sum, zcomp_of_scalar_mul, zcomp_of_sum]
            norm_num [α, sq, mul_def, neg_def]
          rw [h1, h2]
          nth_rewrite 1 [h3]
          ring
...
by
  ...
  | inl h' =>
    ...
    have hx : (x + (-y) * α).x = 1
    · rw [hn]
      simpa using (one_x : (1 : R).x = 1)
    rw [hx'] at hx
    have hy' : (x + (-y) * α).y = -y
    · simp only [Int_Cast_R, neg_def, neg_zero, α, mul_def, MulZeroClass.mul_zero, _root_.add_zero, _root_.mul_one, add_def, _root_.zero_add]
    have hy : (x + (-y) * α).y = 0
    · rw [hn]
      simpa using (one_y : (1 : R).y = 0)
  ...
  | inr h' =>
    ...
    have hx: (x + (-y) * α).x = -1
    · rw[hn]
      simpa [sub_eq_add_neg, α, add_def, neg_def, one_def] using
        show (-1 + α : R).x = -1 by simp [sub_eq_add_neg, α, add_def, neg_def, one_def]
    rw [hx'] at hx
    have hy': (x + (-y) * α).y = -y
    · simp only [Int_Cast_R, neg_def, neg_zero, α, mul_def, MulZeroClass.mul_zero, _root_.add_zero, _root_.mul_one, add_def, _root_.zero_add]
    have hy: (x + (-y) * α).y = 1
    · rw [hn]
      simpa [sub_eq_add_neg, α, add_def, neg_def, one_def] using
        show (-1 + α : R).y = 1 by simp [sub_eq_add_neg, α, add_def, neg_def, one_def]
